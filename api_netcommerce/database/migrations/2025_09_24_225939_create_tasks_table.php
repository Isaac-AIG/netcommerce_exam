...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('tasks', function (Blueprint $table) {
            $table->id(); // Llave primaria de la tarea
            $table->string('task'); // Título de la tarea
            $table->text('description'); // Descripción de la tarea
            $table->date('due_date'); // Fecha de vencimiento de la tarea
            $table->enum('status', ['Pendiente', 'En progreso', 'Retrasado', 'Completado'])->default('Pendiente'); // Estado de la tarea (pendiente, en progreso, retrasado, completado)
